...
def load_and_transform_video(
    ...
    elif video_decode_backend == 'decord' and segment is None:
        decord.bridge.set_bridge('torch')
    ...
    elif video_decode_backend == 'decord' and segment is not None:
        # print(segment)
        decord.bridge.set_bridge('torch')
        decord_vr = VideoReader(video_path, ctx=cpu(0))
        duration = len(decord_vr)
        frame_id_list = np.linspace(segment[0], segment[1], num_frames, dtype=int)
        video_data = decord_vr.get_batch(frame_id_list)
        video_data = video_data.permute(3, 0, 1, 2)  # (T, H, W, C) -> (C, T, H, W)
        video_outputs = transform(video_data)
...
class LanguageBindVideoProcessor(ProcessorMixin):
    ...
    def __call__(self, images=None, text=None, context_length=77, return_tensors=None, segment=None, **kwargs):
        ...
        if images is not None:
            ...
            image_features = [self.image_processor(image, self.transform,
                                                   ...
                                                   segment=segment) for image in images]
